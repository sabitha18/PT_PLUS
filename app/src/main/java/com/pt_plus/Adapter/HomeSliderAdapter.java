...
public class HomeSliderAdapter extends PagerAdapter {
    ...
    public HomeSliderAdapter(Context context, List<HomeSliderModel> homeSliderModel) {
...
//        return images.length;
    ...
    public Object instantiateItem(ViewGroup container, final int position) {
        ...
        View view = layoutInflater.inflate(R.layout.home_slider_layout, null);
        TextView txtTitle = (TextView) view.findViewById(R.id.txt_title);
        txtTitle.setText(homeSliderModel.get(position).title);
        ImageView imageView = (ImageView) view.findViewById(R.id.imageView);
        Glide.with(context).load(homeSliderModel.get(position).imgUrl) .placeholder(R.drawable.no_image)
                .error(R.drawable.no_image).into(imageView);

        AppLogger.log("sdhadkjfhads        "+((homeSliderModel.get(position).imgUrl)));
